...
def maj2(): # permet au joueur de proposer des lettres
    ...
    if guess2 in Liste: #verifie si la reponse proposé figure dans le mot mystere
        print("Bonne réponse")
        print("Vous avez proposé la lettre",guess2)
        for i in range(len(L_indices)): #parcours les indices de la liste L_indices
            indice=L_indices[i] #chaque indice  trouvé se range au fur et a mesure dans la variable "indice"
            L[indice]=guess2 #et on change l'élément de la liste L correspondant a l'indice trouvé par la réponse donné par le joueur
        l=''.join(L) #on change la liste en chaine de caractere et on l'affiche
        print(l)
        print("Il vous reste",essaies,"essaies")#affiche le nombre d'essaies restant
        if l==goal: #vérifie si la liste changée en chaine de caractere correspond au mot attendu
            print("Félicitation, le mot mystere était",goal)
            return rejouer()#renvoie a la fonction (rejouer)
        else:
            return choix_reponse()#renvoie a la fonction (choix_reponse)
